...
$(document).on('change','#partner_id', function() {
    ...
    if($partner_id != $(this).val()) {
        $('#tblCashReceiptDetail').children('tbody').empty();
    }
    $partner_id = $(this).val();
    $.ajax({
        url: $UrlGetDocuments,
...
//        var $document = $partners[$partner_id]['documents'][$document_identity];
        ...
        $html += '<td>';
        $html += '<input type="hidden" name="cash_receipt_details['+$grid_row+'][ref_document_type_id]" id="cash_receipt_detail_ref_document_type_id_'+$grid_row+'" value="'+$document['ref_document_type_id']+'" />';
        $html += '<input type="hidden" name="cash_receipt_details['+$grid_row+'][ref_document_identity]" id="cash_receipt_detail_ref_document_identity_'+$grid_row+'" value="'+$document['ref_document_identity']+'" />';
        ...
        $html += '</td>';
        $html += '<td>';
        $html += '<input type="text" class="form-control" id="cash_receipt_detail_balance_amount_'+$grid_row+'" value="'+$document['outstanding_amount']+'" readonly="true"/>';
        $html += '</td>';
        $html += '<td>';
        $html += '<input onchange="calculateTaxes(this);" type="text" class="form-control fDecimal" name="cash_receipt_details['+$grid_row+'][amount]" id="cash_receipt_detail_amount_'+$grid_row+'" value="0" />';
        $html += '</td>';
        $html += '<td>';
        ...
        $('#cash_receipt_detail_amount_'+$grid_row).trigger('change');
        calculateTotal();
...
$(document).on('click','#btnAddGrid', function() {
    ...
    $html += '</td>';
    $html += '<td>';
    $html += '<input type="text" class="form-control" id="cash_receipt_detail_balance_amount_'+$grid_row+'" value="0.00" readonly="true"/>';
    $html += '</td>';
    $html += '<td>';
...
function calculateTaxes($obj) {
    var $row_id = $($obj).parent().parent().data('row_id');
    var $balance_amount = parseFloat($('#cash_receipt_detail_balance_amount_' + $row_id).val()) || 0.00;
    var $amount = parseFloat($('#cash_receipt_detail_amount_' + $row_id).val()) || 0.00;

    if( ($amount > $balance_amount) && !empty($('#cash_receipt_detail_ref_document_identity_' + $row_id).val()) )
    {
        alert('Total amount must be less than or equal to balance amount.');
        $('#cash_receipt_detail_amount_' + $row_id).val(0)
        $amount = 0;
    }

    // if( $('#cash_receipt_detail_ref_document_identity_' + $row_id).val() != '' ){
    //     var $total_amount = 0;
    //     var $total_rows = 0;

    //     $('#tblCashReceiptDetail tbody tr').each(function() {
    //         $grid_row_id = $(this).data('row_id');
    //         $grid_amount = parseFloat($('#cash_receipt_detail_amount_' + $grid_row_id).val()) || 0.00;
    ...
    //         $('#tblCashReceiptDetail tbody tr').each(function() {
    //             $grid_row_id = $(this).data('row_id');
    //             if( $('#cash_receipt_detail_ref_document_identity_' + $grid_row_id).val() != '' ){
    //                 $divide_amount = ($balance_amount/$total_rows);
    //                 $('#cash_receipt_detail_amount_' + $grid_row_id).val($divide_amount);
    //                 $('#cash_receipt_detail_net_amount_' + $grid_row_id).val($divide_amount)
    //                 $amount = parseFloat($('#cash_receipt_detail_amount_' + $row_id).val()) || 0.00;
    //             }
    //         });
    //     }
    // }
...
function calculateWHTPercent($obj) {
    ...
    $('#cash_receipt_detail_wht_percent_' + $row_id).val($wht_percent);
...
function calculateRowTotal($obj) {
    ...
    $('#cash_receipt_detail_net_amount_' + $row_id).val(roundUpto($net_amount,2));
