...
$(document).on('change','#partner_id', function() {
    ...
    */
    ...
    if($partner_id != $(this).val()) {
        $('#tblBankPaymentDetail').children('tbody').empty();
    }
    $partner_id = $(this).val();
    $.ajax({
        url: $UrlGetDocuments,
...
$(document).on('click','#addRefDocument', function() {
    ...
    if($document_identity != '') {
        ...
        $html += '</td>';
        $html += '<td>';
        $html += '<input type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][document_amount]" id="bank_payment_detail_document_amount_'+$grid_row+'" value="'+$document['document_amount']+'" readonly="true"/>';
        $html += '</td>';
        $html += '<td>';
        $html += '<input type="text" class="form-control fDecimal" id="bank_payment_detail_balance_amount_'+$grid_row+'" value="'+$document['outstanding_amount']+'" readonly="true"/>';
        $html += '</td>';
        // $html += '<td>';
        // $html += '<input type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][document_tax]" id="bank_payment_detail_document_tax_'+$grid_row+'" value="'+$document['document_tax']+'" readonly="true"/>';
        // $html += '</td>'
        $html += '<td>';
        $html += '<input onchange="calculateTaxes(this);" type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][amount]" id="bank_payment_detail_amount_'+$grid_row+'" value="0" />';
        $html += '</td>';
        // $html += '<td>';
        // $html += '<input onchange="calculateDicount(this);" type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][discount_amount]" id="bank_payment_detail_discount_amount_'+ $grid_row +'" value="0" />';
        // $html += '</td>';
        $html += '<td>';
        ...
        $('#bank_payment_detail_amount_'+$grid_row).change(function(){
...
$(document).on('click','#btnAddGrid', function() {
   ...
   if($partner_type_id != '' && $partner_id != '') {
    ...
    $html += '</td>';
    $html += '<td>';
    $html += '<input type="text" class="form-control" id="bank_payment_detail_balance_amount_'+$grid_row+'" value="0.00" readonly="true"/>';
    $html += '</td>';
    // $html += '<td>';
    ...
    // $html += '</td>'
    $html += '<td>';
    $html += '<input onchange="calculateTaxes(this);" type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][amount]" id="bank_payment_detail_amount_'+$grid_row+'" value="0.00" />';
    $html += '</td>';
    // $html += '<td>';
    // $html += '<input onchange="calculateDicount(this);" type="text" class="form-control fDecimal" name="bank_payment_details['+$grid_row+'][discount_amount]" id="bank_payment_detail_discount_amount_'+ $grid_row +'" value="0" />';
    // $html += '</td>';
...
function calculateTaxes($obj) {
    var $row_id = $($obj).parent().parent().data('row_id');
    var $balance_amount = parseFloat($('#bank_payment_detail_balance_amount_' + $row_id).val()) || 0.00;
    var $amount = parseFloat($('#bank_payment_detail_amount_' + $row_id).val()) || 0.00;

    if( ($amount > $balance_amount) && !empty($('#bank_payment_detail_ref_document_identity_' + $row_id).val()) ){
        alert('Total amount must be less than or equal to balance amount.');
        $('#bank_payment_detail_amount_' + $row_id).val(0);
        $amount = 0;
    }

    // if( $('#bank_payment_detail_ref_document_identity_' + $row_id).val() != '' ){
    //     var $total_amount = 0;
    //     var $total_rows = 0;

    //     $('#tblBankPaymentDetail tbody tr').each(function() {
    //         $grid_row_id = $(this).data('row_id');
    //         $grid_amount = parseFloat($('#bank_payment_detail_amount_' + $grid_row_id).val()) || 0.00;
    ...
    //         $('#tblBankPaymentDetail tbody tr').each(function() {
    //             $grid_row_id = $(this).data('row_id');
    //             if( $('#bank_payment_detail_ref_document_identity_' + $grid_row_id).val() != '' ){
    //                 $divide_amount = ($document_amount/$total_rows);
    //                 $('#bank_payment_detail_amount_' + $grid_row_id).val($divide_amount);
    //                 $('#bank_payment_detail_net_amount_' + $grid_row_id).val($divide_amount)
    //                 $amount = parseFloat($('#bank_payment_detail_amount_' + $row_id).val()) || 0.00;
    //             }
    //         });
    //     }
    // }
    ...
    $('#bank_payment_detail_wht_amount_' + $row_id).val($wht_amount);

    var $other_tax_amount = roundUpto(($amount * $other_tax_percent / 100),2);
    $('#bank_payment_detail_other_tax_amount_' + $row_id).val($other_tax_amount);
...
function calculateWHTPercent($obj) {
    ...
    $('#bank_payment_detail_wht_percent_' + $row_id).val($wht_percent);
...
function calculateOtherTaxPercent($obj) {
    ...
    $('#bank_payment_detail_other_tax_percent_' + $row_id).val($other_tax_percent);
...
function calculateRowTotal($obj) {
    ...
    var $row_id = $($obj).parent().parent().data('row_id');
    var $amount = parseFloat($('#bank_payment_detail_amount_' + $row_id).val()) || 0.00;
    // var $discount_amount = parseFloat($('#bank_payment_detail_discount_amount_' + $row_id).val()) || 0.00;
    ...
    $('#bank_payment_detail_net_amount_' + $row_id).val(roundUpto($net_amount,2));
...
function calculateTotal() {
    ...
    $('#tblBankPaymentDetail tbody tr').each(function() {
        ...
        // var $discount_amount = $('#bank_payment_detail_discount_amount_' + $row_id).val() || 0.00;
...
//         $('#bank_payment_detail_discount_amount_' + $row_id).val(roundUpto($discount_ratio,2));
//         calculateRowTotal($('#bank_payment_detail_discount_amount_' + $row_id));
//     });
