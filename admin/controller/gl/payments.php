...
class ControllerGlPayments extends HController {
    ...
    protected function getForm() {
        ...
        $this->data['coa_l3'] = $this->model['coa']->getRows(array('company_id' => $this->session->data['company_id']));
...
//       d($post,true);
    ...
    public function printDocument() {
        ...
        $this->model['ledger'] = $this->load->model('gl/ledger');
        $LedgerDetails = $this->model['ledger']->getTransactionLedger($this->document_type_id,$payment_id);
        ...
        $pdf->Output('Payment Voucher - '.$voucher['document_identity'].'.pdf', 'I');
...
//        d($this->is_ledger,true);
        ...
        if ($this->is_ledger == 0) {
           ...
           // $this->Cell(0, 8, $this->data['address'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
            ...
            $this->SetFont('times','', 8);
            $this->SetFillColor(255,255,255);
            $this->MultiCell(160, 11, html_entity_decode($this->data['remarks']), 0, 'L', 1, 0, '', '', true, 0, false, true, 35, 'T');
