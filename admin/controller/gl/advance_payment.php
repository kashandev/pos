...
class ControllerGlAdvancePayment extends HController {
    ...
    protected function insertData($data) {
        ...
        //$this->model['advance_payment_detail'] = $this->load->model('gl/advance_payment_detail');
        $this->model['ledger'] = $this->load->model('gl/ledger');
...
//    }
    ...
    public function printDocument() {
        $advance_payment_id = $this->request->get['advance_payment_id'];
        ...
        $this->model['mapping_coa'] = $this->load->model('gl/mapping_coa');
        $arrBankAccounts = $this->model['mapping_coa']->getArrays('coa_level3_id','level3_display_name',array('mapping_type_code' => 'CABK'),array('level3_display_name'));

        $this->model['advance_payment'] = $this->load->model('gl/advance_payment');
        $row = $this->model['advance_payment']->getRow(array('advance_payment_id' => $advance_payment_id));
...
//        d(array($row,$arrpartnerTypes,$arrpartners),true);
       ...
       $this->template = 'gl/advance_payment_print.tpl';
...
//        d($contents,true);
        ...
        try
        {
            ...
            // send the PDF
            $html2pdf->Output('AdvancePayment.pdf');
        }
