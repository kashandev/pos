...
//        d(array($filter,$product_stock),true);
    ...
    public function printDocument() {
        ...
        $cashReceiptDetails = $this->model['cash_receipt_detail']->getRows(array('cash_receipt_id' => $cashReceiptId));
            //        d($cashReceiptDetails,true);

        $this->model['ledger'] = $this->load->model('gl/ledger');
        $LedgerDetails = $this->model['ledger']->getTransactionLedger(13,$cashReceiptId);
        ...
        $pdf->data = array(
            ...
            'cost_center_name' => $cashReceipt['cost_center_name'],
            'company_phone' => $company['phone_no'],
            'report_name' => 'Cash Receipt Voucher',
            'partner_name' => $cashReceipt['partner_name'],
...
//        $pdf->SetFont('times', 'B', 10);
        ...
        $pdf->Cell(190, 7, 'CASH RECEIPT VOUCHER', 'T,B', false, 'C', 1, '', 1);
        $pdf->ln(10);
        ...
        $pdf->SetFont('times', '', 8);
        $pdf->Cell(40, 6, $cashReceipt['document_identity'], 0, false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->SetFont('times', 'B', 8);
        $pdf->Cell(20, 6, 'Voucher Date: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');

        $pdf->SetFont('times', '', 8);
        $pdf->Cell(25, 6, stdDate($cashReceipt['document_date']), 0, false, 'L', 0, '', 0, false, 'M', 'M');

        // $pdf->SetFont('times', 'B', 8);
        // $pdf->Cell(17, 6, 'Cost Center: ', 0, false, 'L', 0, '', 0, false, 'M', 'M');

        // $pdf->SetFont('times', '', 8);
        // $pdf->Cell(60, 6, $cashReceipt['cost_center_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->SetFont('times', '', 8);
        $pdf->Cell(60, 6, $cashReceipt['partner_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->SetFont('times','', 8);
        $pdf->SetFillColor(255,255,255);
        $pdf->MultiCell(160, 8, html_entity_decode($cashReceipt['remarks']), 0, 'L', 1, 0, '', '', true, 0, false, true, 40, 'T');
        ...
        foreach($LedgerDetails as $LedgerDetail) {
            /*            $pdf->ln(5);
                        $pdf->Cell(25, 5, $LedgerDetail['account_code'], 0, false, 'L', 0, '', 1);
                        $pdf->Cell(70, 5, html_entity_decode($LedgerDetail['level4_name']), 0, false, 'L', 0, '', 1);
                        $pdf->Cell(30, 5, html_entity_decode($LedgerDetail['remarks']), 0, false, 'L', 0, '', 1);
                        $pdf->Cell(15, 5, '', 0, false, 'L', 0, '', 1);
                        $pdf->Cell(25, 5, number_format($LedgerDetail['debit'],4), 0, false, 'R', 0, '', 1);
                        $pdf->Cell(25, 5, number_format($LedgerDetail['credit'],2), 0, false, 'R', 0, '', 1);
        ...
        /*$pdf->Ln(7);
        $pdf->Cell(140, 7, 'Voucher Total : ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(22, 7, number_format($total_debit,2), 'T,B,B', false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(5, 7, '', 0, false, 'R', 0, '', 0, false, 'M', 'M');
        $pdf->Cell(22, 7, number_format($total_credit,2), 'T,B,B', false, 'R', 0, '', 0, false, 'M', 'M');*/
//        $pdf->Ln(15);
