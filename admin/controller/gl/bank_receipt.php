...
class ControllerGlBankReceipt extends HController {

    protected $document_type_id = 9;

    protected function getAlias() {
        return 'gl/bank_receipt';
    }
    ...
    public function getAjaxLists() {
        ...
        $lang = $this->load->language('gl/bank_receipt');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['bank_receipt_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $this->data['isEdit'] = 1;
            $this->data['document_type_id'] = $this->document_type_id;
            $this->data['document_id'] = $this->request->get['bank_receipt_id'];

            $result = $this->model[$this->getAlias()]->getRow(array('bank_receipt_id' => $this->request->get['bank_receipt_id']));
            // d($result,true);
            ...
            $filter = array(
                'bank_receipt_id' => $this->request->get['bank_receipt_id']
            );
            $details = $this->model['bank_receipt_detail']->getRows($filter,array('sort_order DESC'));
            foreach($details as $detail) {
                ...
                $this->data['bank_receipt_details'][$row_id] = $detail;
                // d($detail,true);
            }
...
//        d($data,true);
        ...
        $bank_receipt_id = $this->model[$this->getAlias()]->add($this->getAlias(), $data);
        $data['document_id'] = $bank_receipt_id;
        //d($data, true);
        ...
        //d($data['bank_receipt_details'], true);
        foreach ($data['bank_receipt_details'] as $sort_order => $detail) {
            $detail['bank_receipt_id'] = $bank_receipt_id;
            ...
            $bank_receipt_detail_id =  $this->model['bank_receipt_detail']->add($this->getAlias(), $detail);

            $gl_data[] = array(
                'document_detail_id' => $bank_receipt_detail_id,
    ...
    protected function updateData($primary_key, $data) {
       ...
       // $this->model['mapping_account'] = $this->load->model('gl/mapping_coa');
        ...
        $this->model['bank_receipt_detail']->deleteBulk($this->getAlias(), array('bank_receipt_id' => $primary_key));
        ...
        $data['base_total_other_tax_amount'] = $data['total_other_tax_amount'] * $data['conversion_rate'];
        $data['base_total_net_amount'] = $data['total_net_amount'] * $data['conversion_rate'];
        $bank_receipt_id = $this->model[$this->getAlias()]->edit($this->getAlias(), $primary_key, $data);
        $data['document_id'] = $bank_receipt_id;
...
//       d($post,true);
        ...
        $details = $post['bank_receipt_details'];
        //d($details,true);
...
//d($Amount,true);
            foreach($Amount as  $ref_document_id => $amount)
            {
                ...
                $filter ['document_id'] = $ID['bank_receipt_id'];
...
//        d(array($filter,$product_stock),true);
    ...
    public function printDocument() {
        ...
        $lang = $this->load->language($this->getAlias());
        $BankReceiptId = $this->request->get['bank_receipt_id'];
        $post = $this->request->post;
        ...
        $BankReceipt = $this->model['bank_receipt']->getRow(array('bank_receipt_id' => $BankReceiptId));
        $TransactionAccount = $this->model['coa']->getRow(array('coa_level3_id' => $BankReceipt['transaction_account_id']));

        $BankReceiptDetails = $this->model['bank_receipt_detail']->getRows(array('bank_receipt_id' => $BankReceiptId));
//        d($BankReceiptDetails,true);


        $this->model['ledger'] = $this->load->model('gl/ledger');
        $LedgerDetails = $this->model['ledger']->getTransactionLedger(9,$BankReceiptId);
        ...
        $pdf->SetTitle('Bank Receipt Voucher');
        $pdf->SetSubject('Bank Receipt Voucher');
        ...
        $pdf->data = array(
            ...
            'report_name' => 'Bank Receipt Voucher',
            'partner_name' => $BankReceipt['partner_name'],
            'cost_center_name' => $BankReceipt['cost_center_name'],
...
//        $pdf->SetFooterMargin(2);
        ...
        $pdf->Cell(190, 7, 'BANK RECEIPT VOUCHER', 'T,B', false, 'C', 1, '', 1);
        $pdf->ln(10);
        ...
        // $pdf->SetFont('times', '', 8);
        // $pdf->Cell(60, 6, $BankReceipt['cost_center_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $pdf->SetFont('times', '', 8);
        $pdf->Cell(60, 6, $BankReceipt['partner_name'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
