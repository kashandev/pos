...
class ControllerGlBankPayment extends HController {

    protected $document_type_id = 8;

    protected function getAlias() {
        return 'gl/bank_payment';
    }
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['bank_payment_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $this->data['isEdit'] = 1;
            $this->data['document_type_id'] = $this->document_type_id;
            $this->data['document_id'] = $this->request->get['bank_payment_id'];

            $result = $this->model[$this->getAlias()]->getRow(array('bank_payment_id' => $this->request->get['bank_payment_id']));
            // d($result);
            ...
            $this->model['bank_payment_detail'] = $this->load->model('gl/bank_payment_detail');
            $filter = array(
                'bank_payment_id' => $this->request->get['bank_payment_id']
            );
            $details = $this->model['bank_payment_detail']->getRows($filter,array('sort_order DESC'));
            // d($details,true);
            foreach($details as $detail) {
                ...
                $this->data['bank_payment_details'][$row_id] = $detail;
            }
...
//        );
    ...
    protected function insertData($data) {
        ...
        $bank_payment_id = $this->model[$this->getAlias()]->add($this->getAlias(), $data);
        $data['document_id'] = $bank_payment_id;
        ...
        //d($data['bank_payment_details'], true);
        foreach ($data['bank_payment_details'] as $sort_order => $detail) {
            $detail['bank_payment_id'] = $bank_payment_id;
            ...
            $bank_payment_detail_id =  $this->model['bank_payment_detail']->add($this->getAlias(), $detail);

            $gl_data[] = array(
                'document_detail_id' => $bank_payment_detail_id,
    ...
    protected function updateData($primary_key, $data) {
        ...
        $this->model['bank_payment_detail']->deleteBulk($this->getAlias(), array('bank_payment_id' => $primary_key));
        ...
        $data['base_total_other_tax_amount'] = $data['total_other_tax_amount'] * $data['conversion_rate'];
        $data['base_total_net_amount'] = $data['total_net_amount'] * $data['conversion_rate'];
        $bank_payment_id = $this->model[$this->getAlias()]->edit($this->getAlias(), $primary_key, $data);
        $data['document_id'] = $bank_payment_id;
...
//       d($post,true);
        ...
        $details = $post['bank_payment_details'];
        //d($details,true);
        if($post['people_id'] && $post['people_id'] != "") {
            ...
            foreach($Amount as  $ref_document_id => $amount)
            {
                ...
                $filter ['document_id'] = $ID['bank_payment_id'];
    ...
    public function printDocument() {
        ...
        $this->model['partner'] = $this->load->model('common/partner');
        $this->model['bank_payment'] = $this->load->model('gl/bank_payment');
        $this->model['bank_payment_detail'] = $this->load->model('gl/bank_payment_detail');
        $this->model['company'] = $this->load->model('setup/company');

        $voucher = $this->model['bank_payment']->getRow(array('bank_payment_id' => $bank_payment_id));
        $details = $this->model['bank_payment_detail']->getRows(array('bank_payment_id' => $bank_payment_id));
        $company = $this->model['company']->getRow(array('company_id' => $session['company_id']));

        $this->model['ledger'] = $this->load->model('gl/ledger');
        $LedgerDetails = $this->model['ledger']->getTransactionLedger(8,$bank_payment_id);
        ...
        $pdf->SetTitle('Bank Payment');
        $pdf->SetSubject('Bank Payment');
...
//        d($this->is_ledger,true);
        ...
        if ($this->is_ledger == 0) {
            ...
            $this->Cell(190, 7, 'BANK PAYMENT VOUCHER', 'T,B', false, 'C', 1, '', 1);
            $this->ln(10);

            $this->SetFont('times', 'B', 8);
            $this->Cell(18, 6, 'Voucher No: ', 0, false, 'R', 0, '', 0, false, 'M', 'M');
            // set font
