...
//		}
        ...
        } elseif(isset($this->request->get['width']) && $this->request->get['width']) {
        ...
        } elseif(isset($this->request->get['height']) && $this->request->get['height']) {
		...
		if (!$json) {
			if (!empty($this->request->files['image']['name']) && is_file($this->request->files['image']['tmp_name'])) {
                ...
                if (!in_array($file_type, $allowed)) {
					$json['error'] = $this->language->get('error_filetype') . $file_type;
				}
		...
		if (!$json) {
            ...
            $thumb_image  = $this->model['image']->resize($filename,$width,$height);
			$json['success'] = true;
			$json['image'] = $filename;
			$json['image_thumb'] = $thumb_image;
		}
