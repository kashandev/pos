...
class ControllerInventoryLength extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/length');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['length_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&length_id=' . $this->request->get['length_id']);
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('length_id' => $id));
            if(count($rows) > 0) {
    ...
    public function validateName()
    {
        ...
        $length_id = $this->request->get['length_id'];

        $this->load->language('inventory/length');
        if ($name) {
            $this->model['length'] = $this->load->model('inventory/length');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND length_id != '".$length_id."'";
            $coa = $this->model['length']->getRow($where);
            if ($coa) {
