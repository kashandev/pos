...
class ControllerInventoryWidth extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/width');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['width_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&width_id=' . $this->request->get['width_id']);
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('width_id' => $id));
            if(count($rows) > 0) {
    ...
    public function validateName()
    {
        ...
        $width_id = $this->request->get['width_id'];

        $this->load->language('inventory/width');
        if ($name) {
            $this->model['width'] = $this->load->model('inventory/width');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND width_id != '".$width_id."'";
            $coa = $this->model['width']->getRow($where);
            if ($coa) {
