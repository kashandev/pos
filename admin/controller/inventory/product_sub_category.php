...
class ControllerInventoryProductSubCategory extends HController {

    protected function getAlias() {
        return 'inventory/product_sub_category';
    }
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/product_sub_category');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['product_sub_category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $result = $this->model[$this->getAlias()]->getRow(array('product_sub_category_id' => $this->request->get['product_sub_category_id']));
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&product_sub_category_id=' . $this->request->get['product_sub_category_id']);
    ...
    public function validateName()
    {
        ...
        $product_sub_category_id = $this->request->get['product_sub_category_id'];

        $this->load->language('inventory/product_sub_category');
        if ($name) {
            $this->model['product_sub_category'] = $this->load->model('inventory/product_sub_category');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND product_sub_category_id != '".$product_sub_category_id."'";
            $coa = $this->model['product_sub_category']->getRow($where);
            if ($coa) {
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('product_sub_category_id' => $id));
           // d(array($id,count($row), $rows), true);
            if(count($rows) > 0) {
    ...
    public function saveProductSubCategory(){
        ...
        $data['company_id'] = $this->session->data['company_id'];
        $this->model['product_sub_category'] = $this->load->model('inventory/product_sub_category');
        $product_sub_category = $this->model['product_sub_category']->getRows(array('company_id' => $this->session->data['company_id']));
        ?>
        <option value="">&nbsp;</option>
        <?php foreach($product_sub_category as $category): ?>
            <option value="<?php echo $category['product_sub_category']; ?>"><?php echo $category['name']; ?></option>
        <?php endforeach;
    }
