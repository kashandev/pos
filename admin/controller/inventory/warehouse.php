...
class ControllerInventoryWareHouse extends HController {
    ...
    public function getAjaxLists() {
        ...
        $data = array();
        $aColumns = array('action', 'code' , 'name', 'address' ,'created_at', 'check_box');
    ...
    protected function insertData($data) {
        ...
        $result = $this->model['warehouse']->getWareHouseCode();
        $result = $result['code'];
        $result++;
        $result = '00'.$result;
        $data['code'] = $result;
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['warehouse_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $result = $this->model[$this->getAlias()]->getRow(array('warehouse_id' => $this->request->get['warehouse_id']));
            foreach($result as $field => $value) {
                $this->data[$field] = $value;
            }
        }

        $this->data['action_validate_code'] = $this->url->link($this->getAlias() . '/validateCode', 'token=' . $this->session->data['token'] . '&warehouse_id=' . $this->request->get['warehouse_id']);
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&warehouse_id=' . $this->request->get['warehouse_id']);
    ...
    public function validateCode()
    {
        ...
        if ($code) {
            ...
            $where .= " AND LOWER(code) = '".strtolower($code)."' AND warehouse_id != '".$warehouse_id."'";
            $coa = $this->model['warehouse']->getRow($where);
            if ($coa) {
    ...
    public function validateName()
    {
        ...
        if ($name) {
            $this->model['warehouse'] = $this->load->model('inventory/warehouse');
            $where = "company_id='" . $company_id . "' AND company_branch_id='" . $company_branch_id . "'";
            $where .= " AND LOWER(name) = '".strtolower($name)."' AND warehouse_id != '".$warehouse_id."'";
            $coa = $this->model['warehouse']->getRow($where);
            if ($coa) {
    ...
    protected function validateDelete() {
        ...
        if (!$this->error) {
            ...
            $lang = $this->load->language('inventory/warehouse');
            foreach($ids as $id) {
                $count = $this->model['stock_ledger']->getCount(array('company_id' => $this->session->data['company_id'], 'warehouse_id' => $id));

                if($count > 0) {
                    $warehouse = $this->model['stock_ledger']->getRow(array('company_id' => $this->session->data['company_id'], 'warehouse_id' => $id));
                    $error[] = sprintf($lang['error_delete'], $warehouse['warehouse'], $count);
                }
            }
