...
class ControllerInventoryThickness extends HController {

    protected function getAlias() {
        return 'inventory/thickness';
    }
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/thickness');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['thickness_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&thickness_id=' . $this->request->get['thickness_id']);
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('thickness_id' => $id));
            if(count($rows) > 0) {
    ...
    public function validateName()
    {
        ...
        $thickness_id = $this->request->get['thickness_id'];

        $this->load->language('inventory/thickness');
        if ($name) {
            $this->model['thickness'] = $this->load->model('inventory/thickness');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND thickness_id != '".$thickness_id."'";
            $coa = $this->model['thickness']->getRow($where);
            if ($coa) {
