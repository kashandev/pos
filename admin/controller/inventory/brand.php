...
class ControllerInventoryBrand extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/brand');
    ...
    public function saveBrand(){
        $data=array();
        $data['company_id'] = $this->session->data['company_id'];
        // $data['name'] = $this->request->get['brand'];
        $this->model['brand'] = $this->load->model('inventory/brand');
        // $this->model['brand']->add('brand', $data);
        $brands = $this->model['brand']->getRows(array('company_id' => $this->session->data['company_id']));
        ?>
        <option value="">&nbsp;</option>
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('brand_id' => $id));
            if(count($rows) > 0) {
    ...
    public function validateName()
    {
        ...
        if ($name) {
            $this->model['brand'] = $this->load->model('inventory/brand');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND brand_id != '".$brand_id."'";
            $coa = $this->model['brand']->getRow($where);
            if ($coa) {
