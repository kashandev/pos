...
class ControllerInventoryGrade extends HController {

    protected function getAlias() {
        return 'inventory/grade';
    }
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/grade');
    ...
    protected function getForm() {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&grade_id=' . $this->request->get['grade_id']);
    ...
    public function validateName()
    {
        ...
        if ($name) {
            ...
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND grade_id != '".$grade_id."'";
            $coa = $this->model['grade']->getRow($where);
            if ($coa) {
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('grade_id' => $id));
            if(count($rows) > 0) {
