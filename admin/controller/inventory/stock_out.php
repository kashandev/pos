...
class ControllerInventoryStockOut extends HController {
    ...
    public function getAjaxLists() {
        $lang = $this->load->language('inventory/stock_out');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['stock_out_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            $this->data['stock_out_details'] = $this->model['stock_out_detail']->getRows(array('stock_out_id' => $this->request->get['stock_out_id']));
        }
    ...
    protected function insertRedirect($id, $data) {
        $url = $this->getURL();
        $this->redirect($this->url->link($this->getAlias().'/update', 'token=' . $this->session->data['token'] . '&stock_out_id=' . $id, 'SSL'));
    }

    protected function updateData($primary_key, $data) {
        //d($data, true);
        $stock_out_id = $primary_key;
        $data['document_date'] = MySqlDate($data['document_date']);
        ...
        $this->model['stock_out']->edit($this->getAlias(), $primary_key, $data);
        $this->model['stock_out_detail']->deleteBulk($this->getAlias(), array('stock_out_id' => $stock_out_id));
        $this->model['document']->deleteBulk($this->getAlias(), array('document_id' => $stock_out_id));
        $this->model['ledger']->deleteBulk($this->getAlias(), array('document_id' => $stock_out_id));
        $this->model['stock_ledger']->deleteBulk($this->getAlias(), array('document_id' => $stock_out_id));
        ...
        $stock_ledger = array();
        $sort_order = 0;
        foreach ($data['stock_out_details'] as $detail) {
            $detail['stock_out_id'] = $stock_out_id;
            $detail['fiscal_year_id'] = $this->session->data['fiscal_year_id'];
            $detail['company_branch_id'] = $this->session->data['company_branch_id'];
            $detail['company_id'] = $this->session->data['company_id'];
    ...
    public function printDocument() {
        ...
        $this->model['stock_out'] = $this->load->model('inventory/stock_out');
        $row = $this->model['stock_out']->getRow(array('stock_out_id' => $stock_out_id));
        ...
        $this->model['stock_out_detail'] = $this->load->model('inventory/stock_out_detail');
        $details = $this->model['stock_out_detail']->getRows(array('stock_out_id' => $stock_out_id));
        foreach($details as $row_no => $detail) {
            $this->data['details'][$row_no] = $detail;
        }
        //d($row,$detail,true);
        $this->template = 'inventory/stock_out_print.tpl';
