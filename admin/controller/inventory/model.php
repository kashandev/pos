...
class ControllerInventoryModel extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/model');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['model_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $result = $this->model[$this->getAlias()]->getRow(array('model_id' => $this->request->get['model_id']));
            foreach($result as $field => $value) {
                $this->data[$field] = $value;
            }
        }

        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&model_id=' . $this->request->get['model_id']);
        $this->data['strValidation']="{
            'rules':{
                'name': {'required':true, 'minlength': 3, 'remote':  {url: '" . $this->data['action_validate_name'] . "', type: 'post'}},
            },
        }";
        $this->data['url_add_model'] = $this->url->link('inventory/brand/saveModel', 'token=' . $this->session->data['token'] );
    ...
    public function saveModel(){
        $data=array();
        $data['company_id'] = $this->session->data['company_id'];
        //$data['name'] = $this->request->get['model'];
        $this->model['model'] = $this->load->model('inventory/model');
        //$this->model['model']->add('model', $data);
        $models = $this->model['model']->getRows(array('company_id' => $this->session->data['company_id']));
        ?>
        <option value="">&nbsp;</option>
    ...
    public function validateName()
    {
        ...
        if ($name) {
            $this->model['model'] = $this->load->model('inventory/model');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND model_id != '".$model_id."'";
            $coa = $this->model['model']->getRow($where);
            if ($coa) {
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('model_id' => $id));
