...
class ControllerInventoryUnit extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/unit');
        $this->model[$this->getAlias()] = $this->load->model($this->getAlias());
        $data = array();
        $aColumns = array('action','code','name','created_at','check_box');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['unit_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $result = $this->model[$this->getAlias()]->getRow(array('unit_id' => $this->request->get['unit_id']));
            foreach($result as $field => $value) {
                $this->data[$field] = $value;
            }
        }

        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&unit_id=' . $this->request->get['unit_id']);
        $this->data['action_validate_code'] = $this->url->link($this->getAlias() . '/validateCode', 'token=' . $this->session->data['token'] . '&unit_id=' . $this->request->get['unit_id']);
        $this->data['strValidation']="{
            'rules':{
                
                'name': {'required':true, 'minlength': 2, 'remote':  {url: '" . $this->data['action_validate_name'] . "', type: 'post'}},
            },
        }";
    ...
    protected function insertData($data) {
        ...
        $result = $this->model['unit']->getUnitCode();
        $result = $result['code'];
        $result++;
        $result = '00'.$result;
        $data['code'] = $result;
    ...
    public function validateCode()
    {
        ...
        if ($code) {
            $this->model['unit'] = $this->load->model('inventory/unit');
            $where = "company_id='" . $company_id . "' AND LOWER(code) = '".strtolower($code)."' AND unit_id != '".$unit_id."'";
            $coa = $this->model['unit']->getRow($where);
            if ($coa) {
    ...
    public function validateName()
    {
        ...
        if ($name) {
            $this->model['unit'] = $this->load->model('inventory/unit');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND unit_id != '".$unit_id."'";
            $coa = $this->model['unit']->getRow($where);
            if ($coa) {
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('unit_id' => $id));
