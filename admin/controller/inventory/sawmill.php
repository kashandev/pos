...
class ControllerInventorySawmill extends HController {

    protected function getAlias() {
        return 'inventory/sawmill';
    }
    ...
    public function getAjaxLists() {
        $this->load->language('inventory/sawmill');
    ...
    protected function getForm() {
        ...
        if (isset($this->request->get['sawmill_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&sawmill_id=' . $this->request->get['sawmill_id']);
    ...
    protected function validateDelete($id)
    {
        if (!$this->user->hasPermission('delete', $this->getAlias())) {
            ...
            $rows = $this->model['product']->getRows(array('sawmill_id' => $id));
            if(count($rows) > 0) {
    ...
    public function validateName()
    {
        ...
        $sawmill_id = $this->request->get['sawmill_id'];

        $this->load->language('inventory/sawmill');
        if ($name) {
            $this->model['sawmill'] = $this->load->model('inventory/sawmill');
            $where = "company_id='" . $company_id . "' AND LOWER(name) = '".strtolower($name)."' AND sawmill_id != '".$sawmill_id."'";
            $coa = $this->model['sawmill']->getRow($where);
            if ($coa) {
