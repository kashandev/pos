...
class ControllerReportGroundSchedule extends HController {

    protected function getAlias() {
        return 'report/ground_schedule';
    }
    ...
    protected function getList() {
        ...
        $this->model['event'] = $this->load->model('setup/event');
        $this->data['events'] = $this->model['event']->getRows();
        ...
        $rows = $this->model['report_criteria']->getRows(array('report_name' => 'ground_schedule', 'user_id' => $this->session->data['user_id']),array('created_at DESC'));
        $reports = array();
        foreach($rows as $row) {
            $reports[] = array(
        ...
        $this->data['action_filter_ground_schedule'] = $this->url->link('report/ground_schedule/filterReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_pdf_ground_schedule'] = $this->url->link('report/ground_schedule/pdfReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_excel_ground_schedule'] = $this->url->link('report/ground_schedule/excelReport', 'token=' . $this->session->data['token'], 'SSL');

        $this->data['action_save_ground_schedule'] = $this->url->link('report/ground_schedule/saveReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_remove_ground_schedule'] = $this->url->link('report/ground_schedule/removeReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_load_ground_schedule'] = $this->url->link('report/ground_schedule/loadReport', 'token=' . $this->session->data['token'], 'SSL');
    ...
    public function filterReport() {
        ...
        $lang = $this->load->language('report/ground_schedule');
        $this->model['ground_schedule'] = $this->load->model('report/ground_schedule');

        $filter = array();
        if(isset($post['from_date']) && $post['from_date'] != '') {
            $filter[] = "`event_date` >= '". MySqlDate($post['from_date']) . "'";
        }
        if(isset($post['to_date']) && $post['to_date'] != '') {
            $filter[] = "event_date <= '". MySqlDate($post['to_date']) . "'";
        }
        ...
        $rows = $this->model['ground_schedule']->getRows($display_columns,$where, $post['show_empty']);
    ...
    public function pdfReport() {
        ...
        $arrAreas = $this->model['ground_area']->getArrays('ground_area_id', 'ground_area');
        $arrEvents = $this->model['event']->getArrays('event_id', 'event_name');
        ...
        if(isset($post['event_id']) && $post['event_id'] != '') {
            $filter[] = "`event_id` = '". $post['event_id'] . "'";
        }
        if(isset($post['ground_area_id']) && $post['ground_area_id'] != '') {
            $filter[] = "`ground_area_id` = '". $post['ground_area_id'] . "'";
        }
        ...
        $rows = $this->model['ground_schedule']->getRows($display_columns,$where, $post['show_empty']);
        //d(array($post, $filter, $where, $rows), true);
        ...
        if($filter) {
            ...
            if(isset($post['from_date']) && $post['from_date'] != '') {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: right;">'.$lang['from_date'].'</td><td>'. $post['from_date'] . '</td>';
                $header .= '</tr>';
            }
            if(isset($post['to_date']) && $post['to_date'] != '') {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: right;">'.$lang['to_date'].'</td><td>'. $post['to_date'] . '</td>';
                $header .= '</tr>';
            }
            if(isset($post['event_id']) && $post['event_id'] != '') {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: right;">'.$lang['event'].'</td><td>'. $arrEvents[$post['event_id']] . '</td>';
                $header .= '</tr>';
            }
            if(isset($post['ground_area_id']) && $post['ground_area_id'] != '') {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: right;">'.$lang['ground_area'].'</td><td>'. $arrAreas[$post['ground_area_id']] . '</td>';
                $header .= '</tr>';
            }
            if(isset($post['member_id']) && $post['member_id'] != '') {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: right;">'.$lang['member'].'</td><td>'. $arrMembers[$post['member_id']] . '</td>';
                $header .= '</tr>';
            }
            $header .= '</table>';
        }
        ...
        foreach($rows as $row_no => $row) {
            ...
            foreach($display_columns as $column) {
                if($column['column_name']=='amount') {
                    $total_amount += $row[$column['display_name']];
                    $body .= '  <td style="text-align: right;">'.number_format($row[$column['display_name']],2).'</td>' . PHP_EOL;
                } else {
        ...
        $body .= '</tbody>' . PHP_EOL;
        if($amount_column != 0 && count($rows) > 0) {
            $body .='<tfoot>' . PHP_EOL;
            $body .='<tr>' . PHP_EOL;
            $body .= '  <td>&nbsp;</td>' . PHP_EOL;
            foreach($display_columns as $column) {
                if($column['column_name']=='amount') {
                    $body .= '  <td style="font-size: 11px; font-weight: bold; text-align: right;">'.number_format($total_amount,2).'</td>' . PHP_EOL;
                } else {
                    $body .= '  <td>&nbsp;</td>' . PHP_EOL;
                }
            }
    ...
    public function excelReport() {
        ...
        if(isset($post['ground_area_id']) && $post['ground_area_id'] != '') {
            $filter[] = "ground_area_id = '". $post['ground_area_id'] . "'";
            $arrFilters[] = "Ground Area = '". $arrAreas[$post['ground_area_id']] . "'";
        }
        if(isset($post['from_date']) && $post['from_date'] != '') {
            $filter[] = "event_date >= '". MySqlDate($post['from_date']) . "'";
...
//                            ,
    ...
    public function saveReport() {
        ...
        $report = $this->model['report_criteria']->getRow(array('report_name' => 'ground_schedule', 'user_id' => $user_id, 'report_id' => $report_id));
        if(empty($report)) {
            $insertData = array(
                'report_name' => 'ground_schedule',
                'user_id' => $user_id,
                'report_id' => $report_id,
        ...
        $reports = array();
        $rows = $this->model['report_criteria']->getRows(array('report_name' => 'ground_schedule', 'user_id' => $user_id),array('created_at DESC'));
        foreach($rows as $row) {
            $reports[] = array(
                'report_criteria_id' => $row['report_criteria_id'],
                'report_id' => $row['report_id'],
    ...
    public function loadReport() {
        ...
        if($data['event_id'] != '') {
            $this->model['event'] = $this->load->model('setup/event');
            $events = $this->model['event']->getRows();
            $html = '<option value="">&nbsp;</option>';
            foreach($events as $event) {
                $html .= '<option value="'.$event['event_id'].'" '.($event['event_id']==$data['event_id']?'selected="true"':'').'>'.$event['event_name'].'</option>';
            }
            $data['events'] = $html;
        }
        if($data['ground_area_id'] != '') {
            $this->model['ground_area'] = $this->load->model('ground/area');
            $rows = $this->model['ground_area']->getRows();
            $html = '<option value="">&nbsp;</option>';
            foreach($rows as $row) {
                $html .= '<option value="'.$row['ground_area_id'].'" '.($row['ground_area_id']==$data['ground_area_id']?'selected="true"':'').'>'.$row['ground_area'].'</option>';
            }
            $data['areas'] = $html;
        }
        if($data['member_id'] != '') {
            ...
            $html = '<option value="">&nbsp;</option>';
            foreach($rows as $row) {
                $html .= '<option value="'.$row['member_id'].'" '.($row['member_id']==$data['member_id']?'selected="true"':'').'>'.$row['member_name'].'</option>';
            }
            $data['members'] = $html;
        }
