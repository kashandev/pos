...
class ControllerReportAcademyReceivable extends HController {

    protected function getAlias() {
        return 'report/academy_receivable';
    }

    protected function init() {
        $this->model[$this->getAlias()] = $this->load->model('report/academy_receivable');
    ...
    protected function getList() {
        ...
        $rows = $this->model['report_criteria']->getRows(array('report_name' => 'academy_receivable', 'user_id' => $this->session->data['user_id']),array('created_at DESC'));
        $reports = array();
        foreach($rows as $row) {
            $reports[] = array(
        ...
        $this->data['action_filter_academy_receivable'] = $this->url->link('report/academy_receivable/filterReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_pdf_academy_receivable'] = $this->url->link('report/academy_receivable/pdfReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_excel_academy_receivable'] = $this->url->link('report/academy_receivable/excelReport', 'token=' . $this->session->data['token'], 'SSL');

        $this->data['action_save_academy_receivable'] = $this->url->link('report/academy_receivable/saveReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_remove_academy_receivable'] = $this->url->link('report/academy_receivable/removeReport', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['action_load_academy_receivable'] = $this->url->link('report/academy_receivable/loadReport', 'token=' . $this->session->data['token'], 'SSL');
    ...
    public function filterReport() {
        ...
        $lang = $this->load->language('report/academy_receivable');
        $this->model['academy_receivable'] = $this->load->model('report/academy_receivable');
        ...
        if(isset($post['balance_only']) && $post['balance_only'] != 0) {
            $filter[] = "balance_amount > 0";
        }
        if(isset($post['receipt_only']) && $post['receipt_only'] != 0) {
            $filter[] = "receipt_amount > 0";
        }
        $where = implode(' AND ', $filter);

        $rows = $this->model['academy_receivable']->getRows($display_columns,$where);
    ...
    public function pdfReport() {
        ...
        $rows = $this->model['academy_receivable']->getRows($display_columns,$where);
        //d(array($post,$where, $having, $rows), true);
        $header = '';
        $header .= '<table width"100%" cellpadding="3">';
        $header .= '<tr>';
        $header .= '<td width="30%" style="border: 1px solid #000000;font-size: smaller; font-weight: lighter;">';
        if($filter) {
            ...
            if(isset($post['balance_only']) && $post['balance_only'] != 0) {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: left;">'.$lang['balance_only'].': </td><td>TRUE</td>';
                $header .= '</tr>';
            }
            if(isset($post['receipt_only']) && $post['receipt_only'] != 0) {
                $header .= '<tr>';
                $header .= '<td style="width: 40%; text-align: left;">'.$lang['receipt_only'].': </td><td>TRUE</td>';
                $header .= '</tr>';
            }
            $header .= '</table>';
        } else {
        ...
        $pdf->SetTitle('Academy Receivable');
        $pdf->SetSubject('Academy Receivable');
        ...
        $pdf->Output('Academy Receivable:'.date('YmdHis').'.pdf', 'I');
        exit;
    }
    ...
    public function excelReport() {
        ...
        if(isset($post['balance_only']) && $post['balance_only'] != 0) {
            $filter[] = "balance_amount > 0";
            $arrFilters[] = "Balance Only = 'True";
        }
        if(isset($post['receipt_only']) && $post['receipt_only'] != 0) {
            $filter[] = "receipt_amount > 0";
            $arrFilters[] = "Receipt Only = 'True";
        }
        $where = implode(' AND ', $filter);
...
//                            ,
    ...
    public function saveReport() {
        ...
        $report = $this->model['report_criteria']->getRow(array('report_name' => 'academy_receivable', 'user_id' => $user_id, 'report_id' => $report_id));
        if(empty($report)) {
            $insertData = array(
                'report_name' => 'academy_receivable',
                'user_id' => $user_id,
                'report_id' => $report_id,
        ...
        $reports = array();
        $rows = $this->model['report_criteria']->getRows(array('report_name' => 'academy_receivable', 'user_id' => $user_id),array('created_at DESC'));
        foreach($rows as $row) {
            $reports[] = array(
                'report_criteria_id' => $row['report_criteria_id'],
                'report_id' => $row['report_id'],
