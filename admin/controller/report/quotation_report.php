...
class ControllerReportQuotationReport extends HController{
    protected function getAlias() {
        return 'report/quotation_report';
    }
...
//
    ...
    public function printReportExcel()
    {
        ...
        $this->model['delivery_challan_report'] = $this->load->model('report/quotation_report');
        $this->model['delivery_challan_detail'] = $this->load->model('inventory/quotation_detail');
        ...
        if($post['partner_id'] != ''){
            $where .= " AND `vw_ins_quotation_detail`.`partner_id`='$partner_id'";
        }
        else{
            $where .= "";
        }
        ...
        if($product_id != ''){
            $where .= " AND `vw_ins_quotation_detail`.`product_id`='$product_id'";
        }
        else{
            $where .= "";
        }
...
//        exit;
        ...
        $rowCount ++;
        $objPHPExcel->getActiveSheet()->mergeCells('A'.($rowCount).":I".($rowCount));
        $objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, 'Quotation Report');
        $objPHPExcel->getActiveSheet()->getStyle('A'.$rowCount)->applyFromArray(
            array(
                'alignment' => array(
                    'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER,
...
//        exit;
        ...
        header('Content-Disposition: attachment;filename="quotation_report.xlsx"');
...
//        $arrFilter['level'] = $post['level'];
        ...
        $pdf->SetSubject('Quotation Report');
...
//        d($arrRows,true);
        foreach($arrRows as $group_name => $rows)
        {
            ...
            foreach($rows as $quotations) {

                $pdf->SetFont('helvetica', '', 7);

                $pdf->ln(6);

                $pdf->Cell(20, 5,stdDate($quotations['document_date']), 0, false, 'L', 0, '', 1);
                $pdf->Cell(30, 5, html_entity_decode($quotations['document_identity']), 0, false, 'L', 0, '', 1);
                $pdf->Cell(30, 5,$quotations['salesman_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(50, 5,$quotations['partner_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(50, 5, $quotations['product_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(20, 5, $quotations['unit_name'], 0, false, 'L', 0, '', 1);
                $pdf->Cell(20, 5, number_format($quotations['qty'],2), 0, false, 'R', 0, '', 1);
                $pdf->Cell(20, 5, number_format($quotations['rate'],2), 0, false, 'R', 0, '', 1);
                $pdf->Cell(25, 5, number_format($quotations['amount'],2), 0, false, 'R', 0, '', 1);

                $total_quantity += $quotations['qty'];
                $total_amount += $quotations['amount'];
        ...
        $pdf->Output('Quotation Report :'.date('YmdHis').'.pdf', 'I');
...
class PDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(30, 5, 'Salesman Name', 'T,B', false, 'L', 0, '', 0, false, 'M', 'M');
