...
class ControllerTravelService extends HController {
    ...
    public function getAjaxLists() {
        $this->load->language('travel/service');
        $this->model[$this->getAlias()] = $this->load->model($this->getAlias());
        $data = array();
        $aColumns = array('action','service_name','created_at');
    ...
    protected function getForm() {
        ...
        $this->data['action_validate_name'] = $this->url->link($this->getAlias() . '/validateName', 'token=' . $this->session->data['token'] . '&service_id=' . $this->request->get['service_id']);
        $this->data['strValidation']="{
            'rules':{
                'service_name': {'required':true, 'minlength': 3, 'remote':  {url: '" . $this->data['action_validate_name'] . "', type: 'post'}},
            },
        }";
    ...
    public function validateName()
    {
        ...
        if ($service_name) {
            ...
            $arrWhere[] = "LOWER(`service_name`) = '".$service_name."'";
            $arrWhere[] = "`service_id` != '".$service_id."'";
            $where = implode(' AND ', $arrWhere);
            $row = $this->model['service']->getRow($where);
            if ($row) {
