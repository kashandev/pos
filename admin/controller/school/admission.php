...
class ControllerSchoolAdmission extends HController {
    ...
    protected function init() {
        $this->model[$this->getAlias()] = $this->load->model('school/student');
    ...
    protected function getForm() {
        ...
        $this->model['house'] = $this->load->model('school/house');
        ...
        $this->data['houses'] = $this->model['house']->getRows(array('company_id' => $this->session->data['company_id'], 'company_branch_id' => $this->session->data['company_branch_id']), array('sort_order'));
        ...
        if (isset($this->request->get['student_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            $result = $this->model[$this->getAlias()]->getRow(array('student_id' => $this->request->get['student_id']));
            ...
            $this->data['class_sections'] = $this->model['class_section']->getRows();
        }
        ...
        $this->data['href_get_pre_registration'] = $this->url->link($this->getAlias() . '/getPreRegistration', 'token=' . $this->session->data['token'] . '&student_id=' . $this->request->get['student_id']);
        $this->data['href_get_section'] = $this->url->link($this->getAlias() . '/getSection', 'token=' . $this->session->data['token'] . '&student_id=' . $this->request->get['student_id']);
        $this->data['href_get_student'] = $this->url->link($this->getAlias() . '/getStudent', 'token=' . $this->session->data['token'] . '&student_id=' . $this->request->get['student_id']);
    ...
    public function getPreRegistration() {
        $lang = $this->load->language('school/admission');
        ...
        foreach($students as $student) {
            $objStudent[$student['student_id']] = array(
                ...
                'for_class_id' => $student['for_class_id'],
                'for_academic_year_id' => $student['for_academic_year_id'],
    ...
    public function printSlip() {
        ...
        $this->model['branch'] = $this->load->model('school/branch');
        $branch = $this->model['branch']->getRow(array('company_branch_id' => $this->session->data['company_branch_id']));

        $this->model['admission'] = $this->load->model('school/admission');
